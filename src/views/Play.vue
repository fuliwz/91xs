        ...
        <section class="player-column">
          <div class="player-card">
            ...
            <div class="player-wrap">
              <video ref="video" controls class="player" playsinline preload="metadata" :poster="secureUrl(vod.vod_pic)"></video>
...
function applyResume() { const p=readJson('91xs_watch_progress',{})[vod.value?.vod_id], el=video.value; if(!p||!el||p.currentTime<15||p.currentTime>p.duration-5)return; const restore=()=>{el.currentTime=p.currentTime;resumeText.value=`上次看到 ${formatTime(p.currentTime)}`}; if(el.readyState>=1)restore();else el.addEventListener('loadedmetadata',restore,{once:true}) }
function destroyPlayer() { clearInterval(progressTimer);progressTimer=null;saveProgress();if(hls){hls.stopLoad();hls.detachMedia();hls.destroy();hls=null}if(video.value){video.value.pause();video.value.removeAttribute('src');video.value.load()} }

// hls.js normally follows URLs found inside the manifest. The custom loader rewrites
// every manifest, level, fragment and key request to HTTPS before the browser sees it.
...
function initPlayer(url) {
  ...
  if(!el||!source){playerLoading.value=false;playerMessage.value='API 没有返回可播放地址';return}
  ...
  if(isHls&&Hls.isSupported()){
    ...
    hls.on(Hls.Events.ERROR,(_,data)=>{if(token!==currentLoad||!data?.fatal)return;if(data.type===Hls.ErrorTypes.MEDIA_ERROR){hls.recoverMediaError()}else failed()})
    hls.attachMedia(el)
  }else{el.src=source;el.load()}
  progressTimer=setInterval(saveProgress,10000)
}
function selectSource(index){if(!sources.value[index])return;sourceIndex.value=index;initPlayer(sources.value[index].url)}

async function loadData(){const token=++currentLoad;destroyPlayer();vod.value=null;recommend.value=[];sources.value=[];sourceIndex.value=0;resumeText.value='';playerLoading.value=true;playerMessage.value='正在加载视频…';const id=route.params.id;if(!id)return
  try{
    ...
    vod.value=item;sources.value=playSources(item);if(sources.value.length)initPlayer(sources.value[0].url);else{playerLoading.value=false;playerMessage.value='API 返回的视频没有可播放地址'}
